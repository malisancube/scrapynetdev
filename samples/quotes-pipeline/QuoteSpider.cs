...
public class QuoteSpider : Spider<IResponse>
{
    ...
    {
        this.logger = logger;
        Pipelines = new List<Type>
        {
            typeof(UploadPipelineItem),
            typeof(CleanupPipelineItem)
        };
    }
    ...
    public override async Task<object?> ParseAsync(BaseRequest response, CancellationToken cancellationToken = default)
    {
        ...
        if (NextPage != null)
        {
            ...
            response.Url = uri.GetLeftPart(System.UriPartial.Authority) + NextPage; 
